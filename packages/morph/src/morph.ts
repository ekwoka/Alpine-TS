...
export const setupMorph: PluginCallback =
  ...
  (
    ...
    const swapElements = (
      ...
      if (shouldSkip(adding, from)) return;
    ...
    const patchChildren = (
      ...
      const fromKeyHoldovers = {} as Record<string, ChildNode>;

      let currentTo: ChildNode | null | undefined = getFirstNode(to);
      let currentFrom: ChildNode | null | undefined = getFirstNode(from);
      ...
      while (currentTo) {
        ...
        const currentFromNext =
          currentFrom && getNextSibling(from, currentFrom); //dom.next(from, fromChildren, currentFrom))
    ...
    const keyToMap = (els: Iterable<Node>) => {
      ...
      return map as Record<string, ChildNode>;
    };

    const addNodeBefore = <T extends Node>(
      parent: Node | Block,
      node: T,
      beforeMe: Node,
    ): T => {
    ...
    if (!(from as ElementWithXAttributes)._x_dataStack) {
      ...
      toEl._x_dataStack = Alpine.closestDataStack(
        from as ElementWithXAttributes,
      );

      // We will kick off a clone on the root element.
      toEl._x_dataStack &&
        Alpine.cloneNode(from as ElementWithXAttributes, toEl);
    }
...
export const isTextOrComment = (el: Node) => {
...
const isElement = (el: Node): el is Element => el.nodeType === 1;
